...
namespace Click
{
    public class ClickRevenueCalculator
    {
        ...
        public ClickResult Calculate()
        {
            ...
            var critChanceUpgrade = _upgradeProvider.GetUpgrade(EUpgradeType.CriticalChance);
            float criticalChance = critChanceUpgrade?.Effect ?? 0f;
            bool isCritical = Random.Range(0f, 1f) < criticalChance;

            var critProfitUpgrade = _upgradeProvider.GetUpgrade(EUpgradeType.CriticalProfit);
